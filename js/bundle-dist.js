    ...
    var UIController = function() {
        ...
        return {
            ...
            writeEventSummary: function writeEventSummary(events) {
                ...
                var trainingDisplayed = [];
                var trainingType = "general";
                var conventionEvent;
                var conventionOn = false;
                events.forEach(function(event) {
                    var isConvention = false;
                    var tags = event.tags;
                    var classStr = "";
                    var trainingSkip = false;
                    var trainingText = {
                        pm: "PM Training",
                    ...
                    if (event.type === "major") {
                        if (!summaryHTML.major.toUpperCase().includes("YOUNG ACHIEVER") && event.title.toUpperCase().includes("YOUNG ACHIEVER")) {
                            summaryHTML.major += "<div class=\"list-event ".concat(classStr, "\">Young Achiever Awards</div>");
                        } else if (!event.title.toUpperCase().includes("YOUNG ACHIEVER") && !classStr.includes("convention")) {
                            summaryHTML.major += "<div class=\"list-event ".concat(classStr, "\">").concat(event.title, "</div>");
                        }
                    } else if (event.type === "training" && !trainingSkip) {
                        summaryHTML.training += "<div class=\"list-event ".concat(classStr, "\">").concat(trainingText[trainingType], "</div>");
                    } else if (event.type === "seasonal") {
                        summaryHTML.seasonal += "<div class=\"list-event ".concat(classStr, "\">").concat(event.title, "</div>");
                    }
                });
                var conventionDates;
