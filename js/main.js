window.addEventListener("DOMContentLoaded", () => {
  ...
  const UIController = (() => {
    ...
    const DOM = {
      ...
      filterSpans: Array.prototype.slice.call(
        document.querySelectorAll(".filter .btn span")
      ),
    };
    ...
    return {
      ...
      writeConventionBanner: (conventionEvent) => {
        ...
        return `<div class="convention-feature"><i class="fas fa-circle"></i><div class="text"><div class="title">National Convention</div><div class="details">${
          ...
          conventionEvent.venue.city.length > 0
            ...
            : ``
        }</div></div><i class="fas fa-circle"></i></div></div>`;
      },
      ...
      writeCalendarGrid: (month, events) => {
          ...
          events.forEach((event) => {
            ...
            if (
              ...
              eventList += `<div class="day-event ${eventClasses}" data-event-id="${
                ...
                event.type === "seasonal" &&
                event.tags !== null &&
                event.tags.includes("significant")
                  ? `<img class="cal-event-bg-img" src="${
                      event.image.cal !== undefined
                        ? event.image.cal
                        : event.image.full
                    }">`
                  : ""
        ...
        // converts the month number into a string for use in Date
        const monthString = month < 10 ? `0${month}` : `${month}`;

        // create date object of first day of the month
        const dayOne = new Date(`${year}-${monthString}-01`);

        // get the day index of first day of the month
        const dayOneIndex = dayOne.getDay();

        // tracks number of calendar squares that have been created
        let squares = 0;

        // HTML string of calendar grid squares to be appended to calendar grid container
        let HTML = "";

        // this function creates innactive squares for the days of the previous and following month that are visible on the calendar
    ...
    const setupEventListeners = () => {
      ...
      window.addEventListener("click", (e) => {
        ...
        if (e.target.closest("div.day-event") !== null) {
          ...
          const monthEvents =
            dataCtrl.state.events[UICtrl.months[dataCtrl.state.currentMonth]];
              ...
              slideChange: function () {
                ...
                if (this.activeIndex == 0) {
                  slides = [
                    ...
                    UICtrl.DOM.monthSlides[this.activeIndex + 1],
                  ];
